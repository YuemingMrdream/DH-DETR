...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        # src 来自于backbone,src= [1,11935,256](第二维是输入的特征数量)   ,# spatial_shapes 是输入的四层特征图的大小[[102,88],[51,44],[26,22],[13,11]]   level_start_index 是每层开始的索引(0,9876,11220,11792)
        ...
        if self.two_stage:
            ...
            init_reference_out = reference_points
        # 存取init_reference_out, 将其可视化
        # self.visual_init_reference_out(enc_outputs_class,init_reference_out)
        # 存取init_reference_out, 将其可视化

        # decoder   memory来自于encoder           # reference_points 是当前query_embed预测的box （共用参数）     query    query_embed 是query的可学习位置编码向量
        box, cls, mask = self.decoder(tgt, reference_points, memory, spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten)
        ...
        if self.two_stage:
            ...
            # return box, cls, mask, topk_enc_outputs_class, topk_coords_unact # 测试 enc 输出
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, attention_position, attention_weight,
                src_padding_mask=None, attn_mask=None, seed_mask=None,cur_all_mask=1, pred_boxes=None,last_second = 0,high_score_mask =None,cls_o = True
                ):  # ++++++++黄
        # self attention     query_pos 是最开始anchor的编码      # seed_mask 应该是高置信度[1,1000]
        if self.aps is not None: # 最后如果是aps(作者提出的检测头)，则进入，  self.aps 是relation_net
            # tgt.detach_()     # 不反向求导？，此处可以改为反向求导。。。。$$$$$$$$$$$$$$$
            # src.detach_()      # encoder的信息 ，相当于后处理
            # +++++++++++可视化
            # weight_indices = torch.argsort(attention_weight[:, :, 4:8, ...].flatten(-3), dim=-1,
            #                                descending=True)  # [1, 1000, 8, 4, 4] ---[1,1000,8,16]  ？？？？
            # attention_weight = torch.gather(attention_weight.flatten(-2), -1, weight_indices[..., :4*max_point_num]) # [1,1000,8,12]
            # weight = weight/weight.sum(-1).unsqueeze(-1).repeat_interleave(2,dim = -1)  # # weight 层归一化后的权重[1, 1000, 8, 4, 2]
            # attention_position_max = torch.gather(attention_position[:, :, 4:8, ...].flatten(-4, -2), -2,
            #                                   weight_indices[..., :8].unsqueeze(-1).repeat_interleave(2,  dim=-1))  # [1,1000,8,4,2]
            #
            # # attention_position_max = attention_position[:, :, 4:8,...].flatten(-3, -2)
            # tol_ = attention_position[:, :, 4:8,...]*attention_weight[:, :, 4:8, ...].unsqueeze(-1)
            # ave_x = tol_[...,0].flatten(-3).sum(-1)/attention_weight[:, :, 4:8, ...].sum(-1).sum(-1).sum(-1)
            # ave_y = tol_[...,1].flatten(-3).sum(-1)/attention_weight[:, :, 4:8, ...].sum(-1).sum(-1).sum(-1)
            #
            # attention_position_mean = torch.stack((ave_x, ave_y),-1)
            #
            # var_x = torch.sqrt((((attention_position[:, :,4:8,...,0].flatten(-3) - ave_x.unsqueeze(-1).repeat_interleave(64,-1)) ** 2)*(attention_weight[:, :, 4:8, ...].flatten(-3))/attention_weight[:, :, 4:8, ...].sum(-1).sum(-1).sum(-1).unsqueeze(-1)).sum(-1))
            # var_y = torch.sqrt((((attention_position[:, :, 4:8,...,1].flatten(-3) - ave_y.unsqueeze(-1).repeat_interleave(64,-1)) ** 2)*(attention_weight[:, :, 4:8, ...].flatten(-3))/attention_weight[:, :, 4:8, ...].sum(-1).sum(-1).sum(-1).unsqueeze(-1)).sum(-1))
            # attention_var = torch.stack((var_x,var_y),-1)
            # attention_inf = torch.stack((attention_position_mean,attention_var),-2)
            # fpath = f'attention_map_tmp.pt'
            # torch.save(attention_inf.detach(), fpath)
            # # +++++++++++++++++++
            # attention_position.detach()
            # attention_weight.detach()
            tgt, attn_mask = self.aps(tgt, seed_mask.float().unsqueeze(-1), pred_boxes, attention_position, attention_weight, high_score_mask) # +++++黄 # 只用加自己的 feature map就行了？？？有点误解原论文的意思。tgt到底是什么维度
            # tgt 没有 norm  中间有的值三十几  要加norm吗？？？？？？？？？？？？？？？？？？？？？？？
            attn_mask |= (~cur_all_mask).unsqueeze(-1) | (~cur_all_mask).unsqueeze(-2)    # 低中置信度之间关注
            cur_all_mask = cur_all_mask.float().unsqueeze(-1)  #?????
            # for i in range(attn_mask.size(0)):
            #     invalid = ~(attn_mask[i].any(-1))
            #     attn_mask[i, invalid, invalid] = 1
        #     attn_mask = (~attn_mask).repeat_interleave(8, 0)  # 因为有8个heads  只关注iou大于阈值的和高置信度的部分，
        #     q = k = self.with_pos_embed(tgt, query_pos)  # 这个query_pos 并没有更新！！！！在DAB中才有好的效果 ....连接一下attentionmap
        #     tgt2 = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1), attn_mask=attn_mask)[0].transpose(0, 1)    # tgt要加上常量embedding       tgt,没有梯度，tgt2有梯度
        #     # tgt = tgt + self.dropout2(tgt2)    # tgt2 密集，因为v还经过全连接 ，tgt稀疏
        #     # tgt = self.norm2(tgt * cur_all_mask)   #cur_all_mask置信度小于0.7 大于0.05的为1
        else:
            assert cur_all_mask == 1                    #！！！！！！！！！！ 在 relation net中改是相加 ，query_pos是anchor的绝对位置和高宽编码
        ...
        #     tgt = self.norm2(tgt * cur_all_mask)   # 只有低置信度的更新
        #先去掉self-attention部分保持一致


        # 倒数第二层保存参数
        if last_second:
            if cls_o !=0:
                tgt2, cls_out= self.cross_attn(self.with_pos_embed(tgt, query_pos) * cur_all_mask, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask,save_map = True, cls_o= cls_o,dynamic_head =False)
            else:
                tgt2= self.cross_attn(self.with_pos_embed(tgt, query_pos) * cur_all_mask, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask,save_map = True, cls_o= cls_o,dynamic_head =False)
                cls_out =tgt2
        else:
        # cross attention   defomable attention    scr是来自于encoder的特征   reference_points[1,1000,4,4] src[1,11935,256]  src_spatial_shapes 每层的形状
            if cls_o != 0:
        ...
        #cls_out 平行
        cls_out = resave_tgt + self.dropout1(cls_out)
        cls_out = self.norm1(cls_out * cur_all_mask)
        # ffn
        cls_output = self.forward_ffn(cls_out)
        # # +++++
        # fpath = f'tgtcls.pt'
        # torch.save(cls_output.detach(), fpath)
        # # +++++
        # +++++++黄
        attention_position = self.cross_attn.attention_position
        attention_weight = self.cross_attn.attention_weight
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, d_model=256, rectified_attention=0, dense_query=0, aps=0, AMself=0, DeA=1 , ARelation=1, num_query = 1000):
        ...
        self.aps = aps
        self.Amself = AMself                             #--------黄
        self.DeA = DeA                                   #--------黄
        self.ARelation = ARelation                       #--------黄
        self.num_query = num_query                       #--------黄
        for i in range(num_layers - aps, num_layers):    #-----初始化最后几层的aps是relationNet
            _layers[i].aps = RelationNet(d_model, Amself =AMself,num_query= num_query)  #--------黄
    ...
    def forward(self, tgt: Tensor, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None):   # src [1, 11935,256]   是来自encoder的特征
        ...
        # reference_points 是 [1,1000,4] 4是box，reference_points_input [1,1000,4,4] 倒数第二个维度更缩放系数有关src_valid_ratios ，目前都是1
            with torch.no_grad():
                attn_mask = None
                if lid >= self.num_layers - self.dense_query:       # dense_query =0  ??????????????????????????????????????
                    tgt_masks = []
            ...
            output, attention_position, attention_weight, cls_output = layer(output, query_pos, reference_points_input, src, src_spatial_shapes,
                                                                 ...
                                                                 cur_all_mask=cur_all_mask, pred_boxes=reference_points,last_second =last_second, high_score_mask =high_score_mask , cls_o =self.DeA)

            # hack implementation for iterative bounding box refinement
            class_logits = self.class_embed[lid](cls_output)        # 改 cls_output
            if layer.aps is None:
                pred_boxes = self.bbox_embed[lid](output) + inverse_sigmoid(reference_points)    # 预测的是delta
            ...
            if lid >= self.num_layers - self.aps - 1:
                ...
                cur_all_mask = (~seed_mask) & (scores > config.floor_thr)   # ?????? 置信度小于0.7 大于0.05的为1   #   这行代码有问题，可以不要低置信度阈值的部分
                # cascade_mask = torch.stack((cur_all_mask & (scores > 0.05), cur_all_mask & (scores <= 0.05))).transpose(0,1)
                # cascade_mask = torch.stack((cur_all_mask & (scores > 0.05), cur_all_mask & (scores <= 0.05))).transpose(0,1)
