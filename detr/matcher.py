...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        with torch.no_grad():
            ...
            # Final cost matrix
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * -cost_giou   # 加上不同损失的权重
            C = C.view(bs, num_queries, -1).cpu()     # 预测与每个gt匹配的cost 也就是loss
            ...
            if outputs['mask'] is not None:
                valid_mask = (tgt_bbox_xyxy[None, :, 0] < out_bbox[:, None, 0]) \
                           ...
                           & (iou > 0)    # (1000,7) 能否匹配上的资格，可以匹配上就是 true
