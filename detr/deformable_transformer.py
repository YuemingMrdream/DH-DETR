...
class DeformableTransformer(nn.Module):
    ...
    def distinct_encoding_mask(self, topk_enc_outputs_class):
        ...
        #     pred_boxes = topk_coords_unact.sigmoid()
        # if layer.aps is not None:
        #     class_logits[~cur_all_mask] = prev_logits[~cur_all_mask]
        #     topk_enc_outputs_class[~cur_all_mask & ~seed_mask] = float('-inf')
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, attention_position, attention_weight,
        ...
        if self.aps is not None: # 最后如果是aps(作者提出的检测头)，则进入，  self.aps 是relation_net
            ...
            tgt, attn_mask = self.aps(tgt, seed_mask.float().unsqueeze(-1), pred_boxes, attention_position, attention_weight, high_score_mask,prev_scores=prev_scores) # +++++黄 # 只用加自己的 feature map就行了？？？有点误解原论文的意思。tgt到底是什么维度
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, d_model=256, rectified_attention=0, dense_query=0, aps=0, AMself=0, DeA=1 , ARelation=1, num_query = 1000):
        ...
        # # # repeat++++++++++++++++++++++++++++++++++
        # for i in [1,3]:    #-----初始化最后几层的aps是relationNet
        #     _layers[i].aps = RelationNet(d_model, Amself =AMself,num_query= num_query)  #--------黄
        #     _layers[i].norm2 = nn.LayerNorm(d_model)   # 加入aps之后的norm
        # # # repeat++++++++++++++++++++++++++++++++++
    ...
    def forward(self, tgt: Tensor, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        # reference_points 是 [1,1000,4] 4是box，reference_points_input [1,1000,4,4] 倒数第二个维度更缩放系数有关src_valid_ratios ，目前都是1
            ...
            # ++++++++++++++++黄# 第二层扩大感受野
            # if lid == self.num_layers - self.aps:
            #     reference_points_input = reference_points_input*(torch.tensor([1,1,1.5,1.5]).cuda()) # 1,1000,4,4 (倒数第二个4是4层feature map)   扩大倒数第二层的感受野
            # ++++++++++++++++
            # if lid ==2:                                                                              # repeat++++++++++++++++++++++++++++++++++
            #     __ = cur_all_mask                                                                    # repeat++++++++++++++++++++++++++++++++++
            #     cur_all_mask = cur_all_mask.float().unsqueeze(-1) # 完全是补充
            ...
            # # 存query
            # if lid == 2:
            #     torch.save(output, r'cur_tgt.pt')
            # # 存query
            # if lid ==2:                                                                              # repeat++++++++++++++++++++++++++++++++++
            #     cur_all_mask = __# 完全是补充                                                         # repeat++++++++++++++++++++++++++++++++++
            # hack implementation for iterative bounding box refinement
            class_logits = self.class_embed[lid](cls_output)        # 改 cls_output
            #  只有没有aps的才更新位置
            if layer.aps is None:
                # if lid == 2:                                                                         # repeat++++++++++++++++++++++++++++++++++
                #     pred_boxes = self.bbox_embed[lid-1](output) + inverse_sigmoid(reference_points)  # repeat++++++++++++++++++++++++++++++++++
                # else:                                                                                # repeat++++++++++++++++++++++++++++++++++
                pred_boxes = self.bbox_embed[lid](output) + inverse_sigmoid(reference_points)    # 预测的是delta
            ...
            if lid >= self.num_layers - self.aps - 1:
                ...
                if self.ARelation == True:
                    # # 非对称mask++++++++++++++++++++++++++++++++++++based on location 只关注上边的
                    # reference_y_points = pred_boxes[:,:,1]
                    # ylocation_x = reference_y_points.unsqueeze(-1).repeat_interleave(self.num_query, -1).unsqueeze(
                    #     -1)  # (1,1000,.1000.,1)
                    # ylocation_y = reference_y_points.unsqueeze(-1).transpose(-2, -1).repeat_interleave(self.num_query, -2).unsqueeze(
                    #     -1)  # (1,1,1000,1)  # (1,.1000.,1000,1)
                    # # 取x<y的值
                    # asymmetric_mask = ylocation_x < ylocation_y
                    # high_score_mask = asymmetric_mask
                    # #  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                # cur_all_mask = (~seed_mask) & (scores > (lid+1)*config.floor_thr)   # ?????? 置信度小于0.7 大于0.05的为1   #   这行代码有问题，可以不要低置信度阈值的部分
